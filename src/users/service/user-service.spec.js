...
describe('User service -> \'Factory\' test suite', () => {
	describe('Evaluating exported module', () => {
		it('exported object should be a function', () => {
			expect(userServiceFactory).toEqual(expect.any(Function));
		});
	});
	...
	describe('Evaluating module functionality', () => {
		...
		*/
		...
		it('the returned object should contain the update feature', async () => {
			...
			await userService.updateUser({current: 'user'}, {});
			expect(update).toHaveBeenCalledTimes(1);
			expect(update).toHaveBeenCalledWith({current: 'user'}, {}, userRepositoryMock, loggerMock);
		});
	});
});
