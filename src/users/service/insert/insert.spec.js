...
describe('User service -> \'Insert\' test suite', () => {
	...
	it('insert should reject when an user is not passed in', async () => {
		// #1: Check for null
		await expect(insert(null, userRepository, logger, hashMock)).rejects.toThrowError(/(.*)(validation failed)/gi);

		// #2: Check for undefined
		await expect(insert(undefined, userRepository, logger, hashMock)).rejects.toThrowError(/(.*)((validation failed))(.*)/gi);
