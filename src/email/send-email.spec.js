...
describe('Email -> \'Send Email\' test suite', () => {
	...
	it('should resolve when everything is ok', async () => {
		...
		await expect(sendEmail(message, validatorMock, transporterMock, loggerMock)).resolves.toStrictEqual(successSendCB.messageId);
	});
	...
	it('should reject when the email object is not valid', async () => {
		...
		await expect(sendEmail(message, validatorMock, transporterMock, loggerMock)).rejects.toStrictEqual({message: 'blabla'});
	});
	...
	it('should reject when the nodemailer fails', async () => {
		...
		await expect(sendEmail(message, validatorMock, transporterMock, loggerMock)).rejects.toThrowError('Some error');
	});
});
